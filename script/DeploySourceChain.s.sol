 ...
 * @notice Script to deploy the SourceChainRouter on Base Sepolia.
...
contract DeploySourceChain is Script {
    ...
    /**
     * @notice Entry point for the source chain deployment.
     */
    function run() external {
        ...
        SourceChainRouter sourceRouter = new SourceChainRouter(CCIP_ROUTER_BASE_SEPOLIA);
        ...
        _authorizeSourceOnArbitrum(deployerPrivateKey, arbitrumRpcUrl, ccipReceiverOnArbitrum, address(sourceRouter));

        _printSummary(sourceRouter, ccipReceiverOnArbitrum);
    }

    /**
     * @notice Switches to the Arbitrum fork to authorize the new source router on the receiver.
     */
    function _authorizeSourceOnArbitrum(
        ...
        vm.startBroadcast(deployerPrivateKey);
        IApollosCCIPReceiver(ccipReceiverOnArbitrum)
            .setAuthorizedSource(BASE_SEPOLIA_CHAIN_SELECTOR, sourceRouter, true);
        vm.stopBroadcast();
    ...
    /**
     * @notice Displays a deployment summary and necessary environment variables.
     */
